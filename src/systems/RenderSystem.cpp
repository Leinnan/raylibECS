...
RenderSystem::RenderSystem(entt::registry<> &registry) {
    ...
    registry.destruction<Components::MeshRenderer>().connect<&RenderSystem::UnloadEntityTexture>(this);
}
...
void RenderSystem::Update(entt::registry<> &registry, Camera &camera) {
    ...
    registry.view<Components::CylinderRenderer, Components::Transform>().each(
        [&](const auto, const auto &cylinder, const auto &transform) {
            DrawCylinder(transform.pos, cylinder.radiusTop, cylinder.radiusBottom, cylinder.height, cylinder.sides,
                         cylinder.color);
        });
    registry.view<Components::BillboardRenderer, Components::Transform>().each(
    ...
    DrawText("Debug "
             "build",
             10, 10, 12, CLITERAL{230, 102, 1, 255});
    EndDrawing();
}

Vector3 RenderSystem::getCameraPos(const Vector3 &pos, const float &angle, const Vector3 &cameraOffset) {
    return {pos.x + std::cos(PI * angle / 180.f) * cameraOffset.x, pos.y + cameraOffset.y,
